...
// SetCommunityAddress sets the community address
func (k Keeper) SetCommunityAddress(ctx sdk.Context, commAddr sdk.AccAddress) {
	store := ctx.KVStore(k.storeKey)
	b := k.cdc.MustMarshalBinaryLengthPrefixed(commAddr)
	store.Set(types.CommunityKey, b)
}

// GetCommunityAddress returns the community address
func (k Keeper) GetCommunityAddress(ctx sdk.Context) (commAddr sdk.AccAddress) {
	store := ctx.KVStore(k.storeKey)
	b := store.Get(types.CommunityKey)
	if b == nil {
		panic("Previous proposer not set")
	}
	k.cdc.MustUnmarshalBinaryLengthPrefixed(b, &commAddr)
	return commAddr
}
